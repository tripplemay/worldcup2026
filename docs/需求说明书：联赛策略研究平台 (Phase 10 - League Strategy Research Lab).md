...
### 8.1.5 自动进化循环(v2,经三视角对抗评审修正;2026-07-03 落地)
把每日 run 从「重跑固定网格」升级为「根据历史结果生成下一代配置」的进化搜索:
- **三发生器**每代 ≤12 配置:精炼(围绕 incumbent,步长下限=统计分辨率、**证据驱动**收缩)+ LLM 提议(≤4,经七步验证器:parse→strictObject→isFinite→夹紧→**量化**→跨字段→双重去重)+ 种子随机(mulberry32(gen|dataHash))。
- **配对显著性障碍**(核心反过拟合):挑战者 vs incumbent 同 L2 窗逐场 ΔCLV,配对 t>1.28 且 n≥30 才算改进——取代「CLV-t 比大小」,防爬噪声峰;主指标 CLV LCB。
- **状态机**:exploring / exhausted(软停:连续 K=3 代无改进 **且** era 试验≥覆盖下限 60;数据实质变化 ≥30 场或 ≥3% 自动复活,复活首件事 incumbent 同窗重评)/ frozen(硬停:era 试验达 **N_max=120**;数据变化不解除,仅显式新 campaign)。
- **holdout 治理**:锁定 manifest 为唯一切分事实源(`partitionWithLockedHoldout`,数据增长时 L3 绝不漂进 L2);**G6 预算 ≤3 次/campaign**,仅首过 G0–G5 的候选可消耗,按 configHash 缓存;提议者简报(与分析员简报物理分离)只含 L1/L2 粗化信息、零 G6 零台账;rationale 永不回灌任何 prompt。
- **审计**:label 服务端从 configHash 派生(防 join 劫持);append-only evolution-log 存 LLM 原始响应 + 验证器逐条裁决,支持注入式重放;去重键 (configHash,dataHash),跨 era 重评计 N、精确缓存命中不计(协议相应修订)。
- **工程**:globalThis 互斥 + 同日幂等(?force=1 绕过);写序 registry→timeline→ledger→log→state;注册表完整性校验(损坏拒跑防 N 归零);单次 run 墙钟护栏 200s,没烧完下次 cron 续跑;`insufficient-power` 静态标记(数据量不达 G2 门槛时面板明示「功效不足」区别于「证否」)。

### 8.2 LLM = 分析员 + 提议者(提议者/裁判分离)
