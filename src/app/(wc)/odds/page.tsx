...
export default function OddsPage() {
  ...
  const tabCls = (active: boolean) =>
    `flex-1 rounded-lg py-1.5 ${active ? 'bg-brand-500 text-white' : 'text-gray-600 dark:text-gray-400'}`;

  return (
    <div>
      <header className="sticky top-0 z-30 -mx-4 mb-3 bg-lightPrimary/95 px-4 py-3 backdrop-blur dark:bg-navy-900/95">
        <div className="flex items-center justify-between">
          <h1 className="text-lg font-bold text-navy-700 dark:text-white">{t('odds.title')}</h1>
          <QuotaRing quota={quota} />
        </div>
        <div className="mt-2 flex rounded-xl bg-white p-1 text-sm shadow-sm dark:bg-navy-800">
          <button onClick={() => setTab('match')} className={tabCls(tab === 'match')}>
            {t('odds.tabMatch')}
          </button>
          <button onClick={() => setTab('winner')} className={tabCls(tab === 'winner')}>
            {t('odds.tabWinner')}
          </button>
        </div>
      </header>
