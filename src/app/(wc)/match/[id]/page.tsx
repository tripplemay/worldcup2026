...
import { useMatchSummary, useMatchOdds } from 'lib/hooks/useWorldCup';
  ...
  return (
    <div>
      <header className="sticky top-0 z-30 -mx-4 mb-3 flex items-center gap-3 bg-lightPrimary/95 px-4 py-3 backdrop-blur dark:bg-navy-900/95">
        ...
        <h1 className="text-lg font-bold text-navy-700 dark:text-white">比赛详情</h1>
      </header>
      ...
      {summary && (
        <>
          ...
          {odds && <OddsTable m={odds} />}

          {summary.events.length > 0 && (
            <Card extra="mb-3 p-4">
              <div className="mb-2 text-sm font-bold text-navy-700 dark:text-white">进球 / 红黄牌</div>
          ...
          {(summary.homeRoster.length > 0 || summary.awayRoster.length > 0) && (
            <Card extra="mb-3 p-4">
              <div className="mb-2 text-sm font-bold text-navy-700 dark:text-white">首发阵容</div>
