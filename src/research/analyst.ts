...
export function buildAnalystBrief(
  ...
  const epochLines = recent
    .map(
      (e) =>
        `- epoch ${e.epoch}(网格${e.gridSize}/累计N${e.cumulativeTrials}):冠军 ${e.winner.label},OOS gap ${f(
          e.winner.oosGap,
        )},CLV-t ${f(e.winner.oosClvT, 2)},PBO ${f(e.pbo, 3)},DSR ${f(
          e.dsr.dsr,
          3,
        )},三筛 ${e.screen.overall ? '过' : '未过'}`,
    )
    .join('\n');
  ...
  const gauntlet = lastLedger
    ? `最近全 gauntlet:候选 ${lastLedger.label},卡在 ${
        ...
        2,
      )};历史回撤 ${f(lastLedger.evidence.drawdown.historicalMaxDD, 3)} MC95 ${f(
        lastLedger.evidence.drawdown.mc95DD,
        3,
      )} 破产 ${lastLedger.evidence.drawdown.ruinPath ? '有' : '无'}`
    : '尚无全 gauntlet 记录';
  const explored = Array.from(
    new Set(recent.flatMap((e) => e.configs.map((c) => c.label.replace(/[\d.\-]+$/, '')))),
  ).join('、');
